...
module.exports = function serviceSpecsExpand (specs, generator) {
  // Update out of date specs
  specs.hooks = specs.hooks || {};

  const specsServices = specs.services || {};
  const feathersSchemas = serviceSpecsCombine(specs, generator);
...
function expandProperties (feathersSchema, depth = 1) {
  ...
  Object.keys(properties || {}).forEach(fieldName => {
    ...
    if (property.type === 'array') {
      ...
      property.items[0].type = (property.items[0].type || 'string');
      if (property.items[0].type !== 'ID') {
        property.items[0].type = property.items[0].type.toLowerCase();
      }
    }
