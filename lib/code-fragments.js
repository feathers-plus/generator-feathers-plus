...
async function refreshCodeFragments (destinationRoot1) {
  ...
  try {
    ...
    console.log('Generator version %s. Source scan took %ds %dms', version, diffTime[0], Math.round(diffTime[1]/1000000));
  } catch (err) {
      console.log(err);
      throw err;
    }
  ...
  */
  ...
  return (name, defaultCode) => {
    ...
    if (defaultCode) {
      ...
      const spaces = ''.padStart(leadSpaces);
      return [`${genDefaultLeader} ${name}`].concat(defaultCode, `${spaces}${genTrailer}`).join(EOL);
    }
