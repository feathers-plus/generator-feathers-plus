...
async function refreshCodeFragments() {
  console.log('\n...Identifying non-template code\n');

  try {
    const code = await extractProject();
    const fileName = join(cwd, CODE_FRAG_PATH);

    // todo inspector('refreshCodeFragments', code);
    //todo writeFileSync(fileName, JSON.stringify(code, null, 2));
    return code;
  } catch(err) {
    console.log(err);
    throw err;
...
function extractSource(filePath, src) {
  ...
  while (i < len) {
    ...
    if (!doingCustomCode) {
      if (line.substr(0, 8) === '//!code:' && line.indexOf('//!end') && line.indexOf('// !end') === -1) {
