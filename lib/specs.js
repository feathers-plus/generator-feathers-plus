...
function initSpecs (what, info) {
  ...
  specsOptions.ver = stashedSpecs.options.ver || '1.0.0';
  specsOptions.inspectConflicts = stashedSpecs.options.inspectConflicts || false;
  specsOptions.semicolons = stashedSpecs.options.semicolons || true;
  specsOptions.freeze = stashedSpecs.options.freeze || [];
...
function updateSpecs (what, props, whosCalling) {
  ...
  switch (what) {
    case 'all':
      break;
    case 'app':
      stashedSpecs.app.name = props.name;
      stashedSpecs.app.description = props.description;
