...
function updateSpecs(what, props, whosCalling) {
  ...
  switch(what) {
    case 'app':
      ...
      break;
    case 'service':
      const serviceSpecs = stashedSpecs.services[props.name];
      ...
      serviceSpecs.isAuthService = props.isAuthService;
      serviceSpecs.requiresAuth = props.requiresAuth;
      serviceSpecs.graphql = props.graphql;
      break;
    case 'connection':
      const key1 = `${props.database}+${props.adapter}`;
      const connectionSpecs = stashedSpecs.connections[key1] = {};
    ...
    case 'authentication':
      ...
      break;
    case 'graphql':
      const graphqlSpecs = stashedSpecs.graphql = {};
      graphqlSpecs.name = 'graphql';
      graphqlSpecs.path = props.path;
      graphqlSpecs.strategy = props.strategy;
      break;
    case 'middleware':
      const key2 = props.name;
      const middlewaresSpecs = stashedSpecs.middlewares[key2] = {};
