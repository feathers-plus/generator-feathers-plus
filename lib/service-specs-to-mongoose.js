...
module.exports = function serviceSpecsToMongoose (feathersSpec, feathersExtension, depth = 1) {
  ...
  Object.keys(properties).forEach(name => {
    ...
    forceTypeToDateIfFormatContainsDate(property);

    switch (property.type) {
      case 'array':
        items = Array.isArray(property.items) ? property.items : [property.items];
        mongooseProperty = serviceSpecsProcessArray(items, mongooseProperty, mongooseSchema, name);
        break;
      case 'object':
...
function serviceSpecsProcessArray(items, mongooseProperty, mongooseSchema, name) {
    const type = items[0].type;
    mongooseProperty.type = type === 'object' ? [] : [typeEquivalence[items[0].type]];
    if (items[0].enum) {
        mongooseProperty.enum = items[0].enum;
    }
    if (!hasOnlyTypeProperty(items[0])) {
        mongooseProperty.type = mongooseProperty.type[0];
        copyOtherMongooseAttributes(items[0], mongooseProperty);
...
function forceTypeToDateIfFormatContainsDate(property) {
    if (property.format && property.format.indexOf('date') > -1) {
        property.type = 'date';
    }
}
